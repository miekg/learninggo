...
imported (with `import` (((keywords, import)))) package is done with
using the package's name and then a dot.  After (((package, bytes)))
`import "bytes"`
...
with `Test`: `func TestXxx(t *testing.T)` .
...
^[The descriptions are copied from the packages' `go doc`.]
